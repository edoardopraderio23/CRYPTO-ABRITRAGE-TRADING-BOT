...
\usepackage{graphicx}
% Look for figures both in the current folder (Overleaf) and in ../reports/ (local)
...
The remainder of the paper is organised as follows. Section~\ref{sec:plan} states the project plan and Section~\ref{sec:diary} the project diary. Section~\ref{sec:background} reviews triangular arbitrage and crypto market microstructure. Section~\ref{sec:architecture} describes the Triple-Gate architecture. Section~\ref{sec:data} reports data-collection methodology. Section~\ref{sec:methods} details the detection algorithm, feature engineering, and training procedure. Section~\ref{sec:results} presents results. Section~\ref{sec:discussion} interprets findings and discusses limitations. Section~\ref{sec:conclusion} concludes.

% ---------------------------------------------------------------------------
\section{Project plan}
\label{sec:plan}

The project was scoped over a five-week window (April 21 -- May 28, 2026) with three authors assuming complementary roles: \textbf{Andrea Cammarano} (data infrastructure: Scout streamer, PostgreSQL schema, repository layer), \textbf{Giacomo Lanni} (quantitative core: Bellman--Ford detector, Profit Gate, property-based tests), and \textbf{Edoardo Praderio} (ML: Guardian features and training, backtest harness, academic report). Deliverables were defined upfront in \texttt{TECHNICAL\_PLAN.md} (committed April 25): a unified CCXT Pro streamer across four venues, a fee-aware cycle detector, a calibrated LightGBM classifier (Guardian), a three-scenario fee-elasticity backtest, and this academic report. The original plan called for two weeks of continuous data collection; in practice we accelerated to 23 hours of streaming with the ML and backtest phases compressed into the final week. Tooling decisions taken at planning time: Python 3.13, PostgreSQL 16, LightGBM for the production model with Random Forest as interpretable baseline, isotonic regression for probability calibration, and \texttt{hypothesis} for property-based testing of the Brain.
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
    \item \textbf{April 21--27.} Scoping and architecture. Decided on triangular arbitrage as the topic. Drafted \texttt{AGENTS.md} (agentic-project governance) and the Triple-Gate decomposition. Tool: Anthropic Claude for design brainstorming.
    \item \textbf{April 28 -- May 4.} Data infrastructure. Andrea built the Scout streamer (CCXT Pro) and the initial PostgreSQL schema; first end-to-end smoke test on Binance. Tools: CCXT Pro, psycopg2.
    \item \textbf{May 5--11.} Brain modules. Giacomo wrote the Bellman--Ford detector (\texttt{cycles.py}) and Profit Gate (\texttt{profit\_gate.py}) with property-based tests via \texttt{hypothesis}. Decision recorded in Issue \#1: add Bybit as the 4th venue to broaden the opportunity surface.
    \item \textbf{May 12--18.} ML training pipeline. Edoardo implemented Guardian feature engineering and training. Mid-week pivot from a replay-based label to \texttt{raw\_log\_return > 0} after the test window proved insufficient at retail-fee thresholds. Tools: scikit-learn, LightGBM.
    \item \textbf{May 19--27.} Backtest, debugging, report. Rewrote the cycle emitter to enumerate all triangles (the original Bellman-Ford-only version returned zero training data in efficient markets). Ran the three-scenario backtest and discovered the empirical efficient-markets finding. Drafted the LaTeX report. Tools: Claude for backtest design and report drafting; Overleaf for LaTeX compilation.
    \item \textbf{May 28--29.} Documentation polish: AI acknowledgements, project diary, user guide, lessons learned. Tagged \texttt{v0.2.0} release on GitHub; submitted on iCorsi.
\end{itemize}
...
The system implements a five-component pipeline. The dataflow is illustrated in the Triple-Gate paragraphs that follow.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{guardian_pr_curve.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{equity_curves.png}
